...
#############################################################################
def NEWbuildLinkout(idArray, GROUP, catx, passx):
	'''Build the links to RT, INSPIRE records, arXiv PDFs, etc.
	   that later get embedded in the display page.'''
	
	# idArray is a list of dictionaries; each list item has an RT and an INSPIRE id
	bibeditHead = '<a href="https://inspirehep.net/record/edit/?ln=en#state=search&p='
        bibeditHTML = ''
        bibeditLink = ''
        recidlist = ''
	inspireIDs = []
	# idArray is a list of dictionaries, ex., { 'inspireID':inspireID, 'RTid':ticketNum }
	# for convenience, we build a simple inspireIDs list, as in the earlier version of the code
	...
	if inspireIDs == []:
		...
		for i in range(0,len(inspireIDs),GROUP):
			...
			for j in inspireIDs[i:i+GROUP]:
				...
				###DOI
				if len(fieldValueArray['0247_2']) > 0 and fieldValueArray['0247_2'][0] == 'DOI':
					doiv = fieldValueArray['0247_a'][0]
					doiLine = '\n  <div class="doiLine">DOI: <a href="http://dx.doi.org/'+ doiv + '">'+doiv+'</a></div>\n'
				else:
					doiLine = ''

				###PDF
				####NOTE: we won't pick up PDFs like http://www.scielo.br/scielo.php?pid=S0103-97332010000300003&script=sci_arttext , b/c we filter for "pdf" on the metadata returned for 8564.
				if len(fieldValueArray['8564_u']) > 0:
					pdfLinks = ''
					for p in fieldValueArray['8564_u']:
						pdfLinks += '<div class="pdfLink"> <a href="'+ p + '">' + p + '</a></div> '

				pdfLine = '\n   <div class="pdfLine">' + pdfLinks + '</div>\n'
				detailLinks += '\n <div class="articleLine">\n'+titleLine+identifierLinks+doiLine+pdfLine+'\n</div>\n'
			## store the cluster of lines for each bibedit linkout group
			linksOutput += '\n<li><div class="linkCluster">'+bibeditLine + detailLinks+'</div></li>\n'
...
def getInspireRecordMetadata(inspireID):
	'''For a given INSPIRE ID, collect the desired metadata fields and return them.'''
	
	fieldArray = { '0247_2':'stdIDsource','0247_a':'stdID','245__a':'title', '8564_u':'files' }
	fieldValues = {}
	...
	# ThS suggested approach for dealing with the problem of two repeating fields
	#   that correspond (say, a type in one field, and a value in another)
	r=AmendableRecord(get_bibrecord(inspireID))
	for pos, val in r.iterfield('035__a', subfield_filter=('9', 'arXiv')):
...
def NEWgetInspireIDs(list_of_tix, catx, passx):
	'''Get the INSPIRE record IDs associated with the RT tickets numbers provided.'''
	...
	for ticketNum in list_of_tix:
		...
		ticketNum_inspireIDs.append({ 'inspireID':inspireID, 'RTid':ticketNum })
